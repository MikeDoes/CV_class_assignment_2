...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        layers_conv_down = [nn.Conv2d(in_channels = input_size, out_channels= 32, kernel_size = (2,2), stride=1), 
        ...
        layers_conv_up = [nn.Conv2d(in_channels = 256, out_channels= 128, kernel_size = (2,2), stride=1),
                        nn.Conv2d(in_channels = 128, out_channels= 64, kernel_size = (2,2), stride=1),
                        nn.Conv2d(in_channels = 64, out_channels= 32, kernel_size = (2,2), stride=1),
                        nn.Conv2d(in_channels = 32, out_channels= 32, kernel_size = (2,2), stride=1)
        ]
        ...
        layers_unpooling = [nn.MaxUnpool2d((2,2)), nn.MaxUnpool2d((2,2)), nn.MaxUnpool2d((2,2)), nn.MaxUnpool2d((2,2))]
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            normalised_out = self.layers_bn_up[i](conv_output)
            relu_out = self.relu(normalised_out)
